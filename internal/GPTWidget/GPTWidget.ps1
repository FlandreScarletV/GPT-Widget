...
function Save-Manifest($value) {
    if(Test-Path -LiteralPath $manifestPath){
        ...
        if(Test-Path -LiteralPath $previous.runtime){$previous|ConvertTo-Json -Depth 12|Set-Content -LiteralPath (Join-Path $previous.runtime 'widget-runtime.json')}
    }
    if(Test-Path -LiteralPath $value.runtime){$value|ConvertTo-Json -Depth 12|Set-Content -LiteralPath (Join-Path $value.runtime 'widget-runtime.json')}
...
try {
    ...
    if ($shared) {
        ...
        & $node (Join-Path $PSScriptRoot 'src\check-coexistence.mjs') $target
        $coexistenceAllowed = -not $current.patchDisabled -and $LASTEXITCODE -eq 0
        ...
        if ($official.Count -and -not $coexistenceAllowed) { throw '副本隔离补丁检查未通过，请重新准备副本。' }
