...
export function patchWorkTelemetry(bytes){
 ...
 const candidates=[...ar.entries].filter(([n,e])=>!e.unpacked&&n.startsWith('.vite/')&&n.endsWith('.js')&&ar.read(n).toString().includes('Starting local app-server sidecar'));
 if(candidates.length!==1)throw Error('App Server startup not unique');
 const main=candidates[0][0];let mainSource=ar.read(main).toString();
 // The desktop stdio transport resolves its command here, independently of the sidecar.
 const localResolver=/function Wq\(e\)\{[\s\S]*?(?=function Gq\()/;
 const resolver=mainSource.match(localResolver)?.[0];
 if(!resolver||!resolver.includes('executablePath:n,args:Kq()')||!resolver.includes('executablePath:r.executablePath,args:Kq()'))throw Error('Local desktop resolver changed');
 mainSource=mainSource.replace(localResolver,resolver
   .replace('executablePath:e,args:n,source:','executablePath:e,args:__gwWorkArgs(e,n),source:')
   .replace('executablePath:n,args:Kq()','executablePath:n,args:__gwWorkArgs(n,Kq())')
   .replace('executablePath:r.executablePath,args:Kq()','executablePath:r.executablePath,args:__gwWorkArgs(r.executablePath,Kq())'));
 const pattern=/let\{args:([\w$]+),command:([\w$]+),cwd:([\w$]+)\}=this\.options,([\w$]+)=\(async\(\)=>\{this\.logger\.info\(`Starting local app-server sidecar`/g;
 if([...mainSource.matchAll(pattern)].length!==1)throw Error('App Server startup shape changed');
